...
- 모바일 메인 카테고리 TOP 10을 한 화면에 3개씩 표시하고, 화살표 클릭 시 3개 단위로 이동하도록 개선했습니다.
- 모바일 카드 열 너비를 고정해 TOP 10 전체가 한 화면에 축소되지 않고 정확히 3개만 노출되도록 보완했습니다.
- 모바일 메인 카테고리의 화살표 옆에도 전체보기 링크가 표시되도록 수정했습니다.
- 모바일 공통 헤더의 드라마·관리자 메뉴를 헤더 내부에 배치해 broadcasts 검색 영역과 겹치지 않도록 수정했습니다.
- 모바일 공통 헤더의 로그인·사용자 버튼을 우측 정렬하고 사용자 버튼을 간결하게 표시했습니다.
- 모바일 드라마 카테고리 메뉴를 화면 너비 안의 2열 스크롤 메뉴로 표시해 잘림을 방지했습니다.

## 2026-09-22

- 기존 최신 `drama/{documentId}/items` 데이터를 유지하면서 `catalogItems`와 `catalogState/drama/items` 구조로 복제하는 멱등 마이그레이션 스크립트를 추가했습니다.
- broadcasts의 드라마 목록 API가 `catalogState/drama/items`의 정렬·참조 정보와 `catalogItems`의 작품 데이터를 조합해 반환하도록 변경했습니다.
- 모든 카테고리의 수동 생성 API가 새 세대 컬렉션을 만들지 않고 `catalogItems`의 신규·변경 작품과 `catalogState/{category}/items`의 현재 상태만 갱신하도록 변경했습니다.
- broadcasts 목록 API가 모든 카테고리에서 새 `catalogState` 구조를 우선 조회하고, 아직 이전되지 않은 카테고리는 기존 구조를 사용하도록 확장했습니다.
- 한국영화, 외국영화, 일반애니, 예능의 기존 최신 카탈로그를 원본 삭제 없이 `catalogItems`와 `catalogState/{category}/items`로 복제하는 마이그레이션을 추가했습니다.
- 크롤러 공통 인터페이스와 선택 레이어를 추가하고 TVHot 구현을 사이트별 어댑터로 분리했습니다.
- `nooo34.tv`의 프로그램 목록·페이지 수·상세 정보를 기존 카탈로그 타입으로 수집하는 전용 크롤러를 추가했습니다.
- 카탈로그 작품 중복 기준을 소스별 상세 주소에서 카테고리와 정규화된 제목 조합으로 변경했습니다.
- 관리자 카탈로그 목록 썸네일에 DB 이미지 데이터 또는 외부 URL 사용 여부를 표시하도록 개선했습니다.
- 썸네일 이미지 저장이 이전 카테고리 문서 대신 현재 `catalogState`와 `catalogItems`를 대상으로 처리되도록 수정했습니다.
- 개별 및 전체 등장인물 업데이트가 현재 `catalogState`에 연결된 `catalogItems`를 대상으로 처리되도록 수정했습니다.
- `catalogItems`의 `thumbnailUrl` 값을 `posterUrl`에 안전하게 복사하는 일회성 백필 스크립트를 추가했습니다.
